...
export default function Borrowings() {
    ...
    const activeItems = getActiveItems('borrowing');
    const archivedItems = getArchivedItems('borrowing');
    ...
    return (
        <div className="w-full flex justify-center">
            <div className="w-full max-w-lg">
                <Logo />
                {loading ? (
                    <p className="mt-20 text-center font-body text-[26px] text-menu">Loading borrowings...</p>
                ) : (
                    <ListContainer
                        title={isArchivedView ? "Archived Borrowings" : "Borrowings"}
                        items={currentItems}
